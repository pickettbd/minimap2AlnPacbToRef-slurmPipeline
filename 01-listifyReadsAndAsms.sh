...
Similarly, some helpful variables should be included in the data/vars-longAln.list file. This should take the form of a bash script format file that can be directly sourced in whatever other script is being run. So, each line should be: VARIABLE="value". An example file is located in the repository with the filename example_vars-longAln.list. $SPECIES and $PROJECT are variables for creating meaningful SLURM job names. $PAForSAM is a way to communicate to the scripts that you want the output to be in PAF or SAM format. The options are "paf" or "sam".
'
